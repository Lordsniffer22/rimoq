...
async def add_user_to_server(server, username, password, days, bot_user_id):
    ...
    if ssh:
        try:
            ...
            #Normal way that displays users in the udp menu
            ...
            #Anonymous way
            useradd_command1 = (f'sudo useradd -M -s /sbin/nologin -e {expiration_date_str} '
                   f'-K PASS_MAX_DAYS={days} -c "GG, {password}" {username} && '
                   f'echo "{username}:{password}" | sudo chpasswd')


            stdin, stdout, stderr = ssh.exec_command(useradd_command1)
